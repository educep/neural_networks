...
from coder.settings import PROJ_ROOT

# List of Django apps to serialize (set to None to serialize everything)
APPS_TO_SERIALIZE = [
    "backend",
    "frontend",
]  # Replace with your actual app names or set to None to include all
...
def build_dependencies_files(list_files: list) -> dict:
    ...
    gitignore = load_gitignore(PROJ_ROOT)
    ...
    for ff in list_files:
        ...
        if ff.endswith((".py", ".html")):
            ...
            if content:
                return {"path": ff.replace(str(PROJ_ROOT), "."), "content": content}
        return content
...
def main():
    ...
    file_structure = get_file_structure(base_path, gitignore, APPS_TO_SERIALIZE)

    output_file_name = (
        os.path.basename(os.path.normpath(base_path)) + "_serialized.json"
    )
    save_to_json(file_structure, output_file_name)

    print(f"Data has been serialized and saved to {output_file_name}")
